...
** A simple Server
   ...
   1. compile the server program:
        gcc -lpthread -o server server-thread-2021.c server-thread-main-2021.c && ./server 17400
...
void callPrintBoard(socket_info *sockInfo);
void iterateGame (socket_info *sockInfo);
Game *createGame();
...
void sendIfWin(socket_info *sockInfo);
...
void sendBoard(char board[3][3], int sockID);
...
int main(int argc, char *argv[]) {
   ...
   while(1) {
      ...
      sockInfo->allPlayers[0] = 'X';
      sockInfo->allPlayers[1] = 'O';
      
      if( pthread_create( &thread_id , NULL ,  start_subserver, (void*) sockInfo) < 0) {
...
/*
      ...
      Need to work on sending the server that the game ended, 
      and free-ing the game so another one can be made.

      

 not needed yet
void callPrintBoard(socket_info *sockInfo) {
   sendBoard(sockInfo->currGame->board, sockInfo->socket_id);
   printf("client requested board\n");
}
*/
...
void sendBoard(char board[3][3], int sockID) {
   for(int i = 0; i <= 2; i++) {
      for(int j = 0; j <=2; j++) {
         send(sockID, &board[i][j], sizeof(char), 0);
      }
   }
}
...
void iterateGame (socket_info *sockInfo) {
   ...
   if (sockInfo->currGame->player == sockInfo->allPlayers[sockInfo->socket_id - 4]) {
      currPlayer++;
      send(sockInfo->socket_id, &currPlayer, sizeof(int), 0);
      nextMove(sockInfo);
      oppositeSocket = (oppositeSocket == sockInfo->socket_id) ? 5 : 4;
      sendBoard(sockInfo->currGame->board, sockInfo->socket_id);
   } else if (sockInfo->currGame->gameOver == 1){ 
      currPlayer--;
      send(sockInfo->socket_id, &currPlayer, sizeof(int), 0);
      sendBoard(sockInfo->currGame->board, sockInfo->socket_id);
   } else if (sockInfo->currGame->rounds == 9){
...
void nextMove (socket_info *sockInfo){
	...
	if (sockInfo->currGame->gameOver == 1){
      printf("Player %c Wins!", sockInfo->currGame->player);
      free(sockInfo->currGame);
...
void sendIfWin(socket_info *sockInfo) {
   ...
   send(sockInfo->socket_id, &sockInfo->currGame->gameOver, sizeof(int), 0);
}
/* dont change */
Game *createGame() {
